    ...
    foreach ($gallery_images as $image_id) {
      ...
      if (strpos($image_title, 'achterkant') !== false) {
        $back_image = wp_get_attachment_image($image_id, 'full', false, [
          'class' => 'w-full h-full object-contain absolute inset-0 opacity-0 group-hover:opacity-100 transition duration-300 transform-gpu will-change-transform rounded-[20px]'
        ]);
        break;
      }
    }
  ?>
    <div class="group">
      <div class="aspect-[4/5] rounded-[20px] overflow-hidden bg-[url('/wp-content/themes/fastfoot-style/assets/images/mesh-pattern.png')] bg-cover">
        <a href="<?php the_permalink(); ?>" class="block w-full h-full relative rounded-[20px] overflow-hidden">
          <?php 
            echo $product->get_image('full', [
              'class' => 'w-full h-full object-contain transition duration-300 group-hover:opacity-0 transform-gpu will-change-transform rounded-[20px]'
            ]);
